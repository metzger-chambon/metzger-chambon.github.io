...
export default function StudyCard({ study }: StudyCardProps) {
  const getImageUrl = (study: Study) => {
    ...
    */
    ...
    return defaultImages[study.biologicalApplication as keyof typeof defaultImages] || 
  ...
  return (
    <Card className="overflow-hidden hover:shadow-lg transition-shadow">
      ...
      <div className="flex items-center space-x-2 mb-3">
          <Badge className={getBiologicalApplicationColor(study.categories.biologicalApplication)}>
            {study.categories.biologicalApplication}
          </Badge>
          <Badge className={getPlatformNameColor(study.categories.sequencingPlatform.name)}>
            {study.categories.sequencingPlatform.name}
          </Badge>
          {study.categories.sequencingPlatform.sub && (
            <Badge className={getPlatformSubColor(study.categories.sequencingPlatform.sub)}>
              {study.categories.sequencingPlatform.sub}
            </Badge>
          )}
        </div>
